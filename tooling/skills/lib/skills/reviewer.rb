...
module Skills
  ...
  class Reviewer
    ...
    MODEL = "mimo-v2.5"
    CRITERIA = %w[self-containment technique-vs-reference action-orientation conciseness routing-quality].freeze
    SEVERITIES = %w[error warning advice].freeze
    ...
    def chat(api_key)
      config = @ruby_llm::Configuration.new
      ...
      @ruby_llm::Context.new(config).chat(model: MODEL, provider: :openai, assume_model_exists: true)
              ...
              .with_provider_options(thinking: { type: "disabled" })
    ...
    def untrusted_skill_prompt(skill, source)
      ...
      <<~PROMPT
        ...
        [BEGIN UNTRUSTED SKILL.md: #{skill.path.join("SKILL.md")}]
        #{source}
    ...
    def finding_from(item, source)
      ...
      unless SEVERITIES.include?(severity) && CRITERIA.include?(criterion) && quote.is_a?(String) && !quote.empty? && suggestion.is_a?(String) && !suggestion.empty?
        raise Error, "review provider returned an invalid finding"
      end
